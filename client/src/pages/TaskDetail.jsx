...
const Activities = ({ activity, id, refetch }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Submitting activity:", { data, id });
    ...
    return (
      <div className={`flex space-x-4`}>
        ...
        <div className='flex flex-col gap-y-1 mb-8'>
          <p className='font-semibold'>{item?.by?.name || "Unknown"}</p>
      ...
      <div className='w-full md:w-1/2'>
        ...
        <div className='w-full space-y-0'>
          {activity?.length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-10">No activities yet. Add your first activity!</p>
          )}
        </div>
      </div>
...
const TaskDetail = () => {
  ...
  return (
    <div className='w-full flex flex-col gap-3 mb-4 overflow-y-hidden'>
      <h1 className='text-2xl text-gray-600 font-bold'>{task?.title || "Task Details"}</h1>
              ...
              <div className='w-full md:w-1/2 space-y-8'>
                <div className='flex items-center gap-5'>
                  ...
                  >
                    ...
                    <span className='uppercase'>{task?.priority || "Normal"} Priority</span>
                  </div>

                  <div className={clsx("flex items-center gap-2")}>
                    <TaskColor className={TASK_TYPE[task?.stage]} />
                    <span className='text-black uppercase'>{task?.stage || "Todo"}</span>
                  </div>
                </div>

                <p className='text-gray-500'>
                  Created At: {task?.date ? new Date(task.date).toDateString() : "Date not set"}
                </p>
                          ...
                          <div className='space-y-1'>
                            <div className='flex gap-2 items-center'>
                              <span className='text-sm text-gray-500'>
                                {el?.date ? new Date(el.date).toDateString() : "No date"}
                              </span>
              ...
              <div className='w-full md:w-1/2 space-y-3'>
                {task?.description && (
                  <div className='mb-10'>
                    ...
                    <div className='w-full whitespace-pre-wrap'>{task?.description}</div>
                  </div>
                )}
